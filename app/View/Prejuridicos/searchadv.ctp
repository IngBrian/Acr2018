...
<?php echo $this->Form->create("Prejuridico" ,['class'=>'form-horizontal','action' => 'searchadv']);?> 
      <div class="col-sm-7">  
             ...
             <? echo $this->Form->input('ordenante_id', array('options'=> $ordenantes, 'type' => 'select', 'empty' => '<< Seleccione  Otorgnate I >>', 'label' =>false, 'id' => 'ordenante_criteria','placeholder'=>'Selecione Ordenante','class'=>'form-control')) ?>
              <? echo $this->Form->input('cliente_id', array('options'=> $cliente, 'type' => 'select', 'empty' => '<< Seleccione  Otorgnate II >>', 'label' =>false, 'id' => 'cliente_criteria','placeholder'=>'Selecione Demandado','class'=>'form-control')) ?>
             ...
             <?= $this->Form->input('tproceso_id', array('options' => $tprocesos, 'type' => 'select', 'empty' => '<< Seleccione Tipo de Acto >>', 'label' => false, 'id' => 'tproceso','class'=>'form-control')) ?>
             ...
             <?= $this->Form->input('ubicacion_id', array('options' => $ubicaciones, 'type' => 'select', 'empty' => '<< Seleccione Ubicacion >>', 'label' => false, 'id' => 'ubicacion','class'=>'form-control')) ?> 
              ...
              <?= $this->Form->input('pagaduria_id', array('options' => $pagadurias, 'type' => 'select', 'empty' => '<< Seleccione Relacionado >>', 'label' => false, 'id' => 'pagaduria','class'=>'form-control')) ?>
               <?= $this->Form->input('pendiente_id', array('options' => $pendientes, 'type' => 'select', 'empty' => '<< Seleccione Notario (E) >>', 'label' => false, 'id' => 'pendiente','class'=>'form-control')) ?>
    
             <?= $this->Form->input('juzgado_id', array('options' => $juzgados, 'type' => 'select', 'empty' => '<< Seleccione Entidad >>', 'label' => false, 'id' => 'juzgado','class'=>'form-control')) ?>
