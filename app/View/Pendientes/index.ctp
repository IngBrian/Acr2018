...
<div class="container"> 
    ...
    <?php 
        echo $this->Form->create("Pendiente", ['action' => 'search','class'=>'form-inline']);
        $options = array(
            'nombre' => 'Nombre'
        );
    ?>
...
<br>
  ...
  <tbody>
        <?php  foreach($pendientes as $pendiente) : ?>
          <tr>
            <td id=<?=$var?> style="cursor: pointer;" onClick="reply_click(<?=$var?>,<?=$var2?>)" ><?= $pendiente['Pendiente']['nombre'] ?></td>
            <?php
            ...
            <p align="justify"><b>NOMBRE </b><?php echo $nombre ?></p>
             <?php $var=$var+1;$var2=$var2+1;?>
                ...
                <?php echo $this->Form->postLink(
                      'Eliminar',
                      array('controller' => 'Pendientes', 'action' => 'delete', $pendiente['Pendiente']['id']),
                      array('confirm' => 'Esta Usted seguro que desea eliminar este registro?','class'=>'btn btn-default navbar-btn'));?>

                      <?php echo $this->Html->link(
                      'Editar',
                      array('controller' => 'Pendientes', 'action' => 'edit', $pendiente['Pendiente']['id']),
