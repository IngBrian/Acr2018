...
<div class="container"> 
    ...
    <?php 
        echo $this->Form->create("Subestado", ['action' => 'search','class'=>'form-inline']);
        $options = array(
            'nombre' => 'Nombre'
        );
    ?>
...
<br>
  ...
  <tbody>
        <?php  foreach($subestados as $subestado) : ?>
          <tr>
            <td id=<?=$var?> style="cursor: pointer;" onClick="reply_click(<?=$var?>,<?=$var2?>)" ><?= $subestado['Subestado']['nombre'] ?></td>
            <?php
            $nombre=$subestado['Subestado']['nombre'];
            ?>
            ...
            <p align="justify"><b>NOMBRE </b><?php echo $nombre ?></p>
             <?php $var=$var+1;$var2=$var2+1;?>
                ...
                <?php echo $this->Form->postLink(
                      'Eliminar',
                      array('controller' => 'Subestados', 'action' => 'delete', $subestado['Subestado']['id']),
                      array('confirm' => 'Esta Usted seguro que desea eliminar este registro?','class'=>'btn btn-default navbar-btn'));?>

                      <?php echo $this->Html->link(
                      'Editar',
                      array('controller' => 'Subestados', 'action' => 'edit', $subestado['Subestado']['id']),
