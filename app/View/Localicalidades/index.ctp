...
<div class="container"> 
    <h5>Buscador</h5>   
    <?php 
        echo $this->Form->create("Localidade", ['action' => 'search','class'=>'form-inline']);
        $options = array(
            'nombre' => 'Nombre'
        );
    ?>
...
<br>
  ...
  <tbody>
        <?php  foreach($localidades as $localidade) : ?>
          <tr>
            <td id=<?=$var?> style="cursor: pointer;" onClick="reply_click(<?=$var?>,<?=$var2?>)" ><?= $localidade['Localidade']['nombre'] ?></td>
            <?php
            $nombre=$localidade['Localidade']['nombre'];
            ?>
            ...
            <p align="justify"><b>NOMBRE </b><?php echo $nombre ?></p>
             <?php $var=$var+1;$var2=$var2+1;?>
                ...
                <?php echo $this->Form->postLink(
                      'Eliminar',
                      array('controller' => 'Localidades', 'action' => 'delete', $localidade['Localidade']['id']),
                      array('confirm' => 'Esta Usted seguro que desea eliminar este registro?','class'=>'btn btn-default navbar-btn'));?>

                      <?php echo $this->Html->link(
                      'Editar',
                      array('controller' => 'Localidades', 'action' => 'edit', $localidade['Localidade']['id']),
