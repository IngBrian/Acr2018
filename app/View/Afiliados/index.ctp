...
<div class="container"> 
    ...
    <?php 
        echo $this->Form->create("Afiliado", ['action' => 'search','class'=>'form-inline']);
        $options = array(
    ...
    ?>
        ...
        <? echo $this->Form->input('criterio',array('options' => $options, 'type' => 'select', 'empty' => 'Listar Todos', 'label' => false,'class'=>'form-control')) ?> 
        </div>
...
<div class="">
  ...
  <tbody>
		...
		foreach($afiliados as $afiliado) : ?>
          ...
          <td id=<?=$var?> style="cursor: pointer;" onClick="reply_click(<?=$var?>,<?=$var2?>)" >
          <?= $afiliado['User']['nombre_completo'] ?></td>
          

          <?php

          $nombre=$afiliado['User']['nombre_completo'];
          $identificacion=$afiliado['User']['cc_nit'];
          $telefono=$afiliado['User']['email'];
          ...
          <td onClick="reply_click(<?=$var?>,<?=$var2?>)"   style="cursor: pointer;">
              <div  style="display:none;" id=<?=$var2?>  align="left"  >
                ...
                <p align="justify"><b>IDENTIFICACION </b><?php echo $identificacion ?></p>
                <p align="justify"><b>EMAIL </b><?php echo $telefono ?></p>
                <?php $var=$var+1;$var2=$var2+1;?>

                <?php echo $this->Form->postLink(
                      'Eliminar',
                      array('controller' => 'Afiliados', 'action' => 'delete', $afiliado['User']['id']),
                      array('confirm' => 'Esta Usted seguro que desea eliminar este registro?','class'=>'btn btn-default navbar-btn'));?>

                      <?php echo $this->Html->link(
                      'Editar',
                      array('controller' => 'Afiliados', 'action' => 'edit', $afiliado['User']['id']),
