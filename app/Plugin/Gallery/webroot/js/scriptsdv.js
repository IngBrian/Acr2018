...
var Album = {
    init: function (settings) {
         ...
         * @type {string}
         */
        Album.config.fetchUrl = Album.config.baseUrl + '/items/index/' + Album.config.id;
         ...
         * @type {string}
         */
        Album.config.deleteUrl = Album.config.baseUrl + '/items/delete/';
         ...
         * @type {string}
         */
        Album.config.sortUrl = Album.config.baseUrl + '/items/sort';
    ...
    fetch: function () {
        $.getJSON(Album.config.fetchUrl, function (response) {
            ...
            for (var i = 0; i < items.length; i++) {
               var url=Album.getExt(items[i].link);
   ...
   getExt: function (ext){
        ...
        if( (ext.indexOf(".mp4")>1) || (ext.indexOf(".MP4")>1)) {
            return pathIcons+"play.png";

        }else if( (ext.indexOf(".mp3")>1) || (ext.indexOf(".MP3")>1)) {
            return pathIcons+"mp3.png"

        }else{
            return pathIcons+"iError.png"
    ...
    configureDropzone: function () {
        ...
        Drop.on("success", function (r, response) {
            ...
            //debugger;
            //Album.renderPicture(response.picture.Picture.id,response.picture.Picture.styles.medium, null, response.picture.Picture.styles.large);
    ...
    renderPicture: function (id, url, caption, path) {
        var template = Album.config.pictureTmpl.html();
        Mustache.parse(template);

        // Rende the picture using Mustache
        var rendered = Mustache.render(template, {id: id, url: url, caption: caption, path: path});
