...
class Logalbum extends GalleryAppModel
{
    public $name = 'Album';
    public $tablePrefix = 'tbloggallery_';
    public $order = 'LogAlbum.id DESC';
    ...
    {
        if ($created) {
            $this->createFolder($this->data['LogAlbum']['id']);
        }
    }
    ...
    public function getAttachedAlbum($model = null, $model_id = null)
    {
        return $this->find(
            ...
            array(
                'conditions' => array(
                    'LogAlbum.model' => $model,
                    'LogAlbum.model_id' => $model_id
                )
            )
        );
    }
