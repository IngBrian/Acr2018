...
class Logdocument extends GalleryAppModel
{
    public $name = 'Documents';
    public $tablePrefix = 'tbloggallery_';
    public $order = 'LogDocument.id DESC';
    ...
    {
        if ($created) {
            $this->createFolder($this->data['LogDocument']['id']);
        }
    }
    ...
    public function getAttachedDocument($model = null, $model_id = null)
    {
        return $this->find(
            ...
            array(
                'conditions' => array(
                    'LogDocument.model' => $model,
                    'LogDocument.model_id' => $model_id
                )
            )
        );
    }
